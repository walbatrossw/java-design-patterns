...
아래의 그림을 보면 객체 아래에는 점선이 뻗어 나가는데 이것을 "생명선"이라 하고 해당 객체가 존재함을 보여준다. 생명선을 따라서 좁고 긴 사각형이 나올 수 있다.
이 상자를 "활성구간"이라 한다. 이는 실제로 객체가 연산을 실행하는 상태임을 보여준다. 활성 구간의 크기는 실행시간을 고려하여 적당히 설정한다.
